...
for iter in tqdm(range(num_iters-1)):
    ...
    # 3.4 Compute the Lagrangian.
    zeta_t = get_zeta(psi, lambda_t, mu_al)
    #zeta_t[~penetration] = 0.0
    #plt.figure()
    #plt.imshow(zeta_t.T, cmap='gray', origin='lower')
    #plt.show()

    L_t = phi_t_curvature + zeta_t

    # 3.4 Update the signed distance field via advection
    phi_next = phi_t + h * L_t * phi_t_grad_norm
    phi_t_arr[:,:,iter+1] = phi_next

    # 3.5 
    penetration = torch.logical_and((psi < 0), (phi_next < 0))    
    lambda_t_arr[:,:,iter+1][penetration] = (lambda_t - mu_al * psi)[penetration]
